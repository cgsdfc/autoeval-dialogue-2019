...
\begin{document}
    ...
    \begin{align}
        d(u, v) = \frac{\sum_{i,j}\textit{dist}(u[i], v[j])}{|u| \cdot |v|}
    \end{align}
    for point $u[i]$ in cluster $u$ and $v[j]$ in cluster $v$, with $\textit{dist}(\cdot, \cdot)$ as the distance metric. $|u|$ and $|v|$ are the cardinalities of cluster $u$ and $v$, respectively.
