...
\begin{document}
    ...
    \paragraph{ADEM}
    The ADEM \cite{ADEM} metric is powered by a neural network trained on a conversation corpus with human annotated scores. The model first embeds the input into a low-rank vector space and then predict a supervised score label with a linear combination of the embeddings for a context $c$, response $\hat{r}$, and reference $r$.
    \begin{align}
