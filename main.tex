...
\begin{document}
    ...
    To better observe the agreement and disagreement among the metrics, we apply a hierarachical clustering to the columns of the matrix $M$. A hierarachical clustering algorithm works by starting with a forest of nodes and iteratively joining them into larger clusters according to a \emph{metric} that defines the distance of two nodes and a \emph{method} that determines the distance of two clusters. In our case, we use the Pearson'R as the metric and
