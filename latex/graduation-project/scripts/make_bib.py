...
class BibMaker:
    ...
    def extract_bib_entry(self, filename):
        ...
        entry, n_matched = max([self.count_matched_keys(bib)
                                for bib in bib_db.entries], key=operator.itemgetter(1))
        ...
        for file in files:
            ...
            if bib is None:
                ...
                if bib[K_ID] in bib_keys:
                    logging.warning(
                        'duplicate bib entry: %s from %s', bib[K_ID], file)
                    continue
                name = bib[K_ID]
                bib_keys.add(name)
                if self.rename_entry:
                    ...
                    bib[K_ID] = name
                    logging.info(
                        'renaming entry from {} to {}'.format(old_name, name))
    ...
    def remove_keys_not_in_white_list(self, bib: dict):
        excluded_keys = [key for key in bib.keys(
        ) if key.islower() and key not in self.WHITE_LIST]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-o', '--output', help='output file of the merged and cleaned single file')
    parser.add_argument('-white-list-only', action='store_true',
                        help='only keep those keys in the white list')
    parser.add_argument('-stop-at-empty', action='store_true',
                        help='stop when an empty bib file was found')
    parser.add_argument('-rename-entry', action='store_true',
                        help='rename entry key to their filename')
